...
public class OnEnterSupport {
	...
	private static final List<CharacterPair> DEFAULT_BRACKETS = Arrays.asList(new CharacterPair("(", ")"),
			new CharacterPair("{", "}"), new CharacterPair("[", "]"));
	...
	public OnEnterSupport(List<CharacterPair> brackets, List<OnEnterRule> regExpRules) {
		this.brackets = (brackets != null ? brackets : DEFAULT_BRACKETS).stream().map((bracket -> {
