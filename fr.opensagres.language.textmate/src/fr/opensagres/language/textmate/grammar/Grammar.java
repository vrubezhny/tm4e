...
public class Grammar implements IGrammar, IRuleFactoryHelper {
	...
	public ITokenizeLineResult tokenizeLine(String lineText, List<StackElement> prevState) {
		...
		if (prevState == null) {
			...
			prevState.add(new StackElement(this._rootId, -1, null, this.getRule(this._rootId).getName(null, null), null));
		} else {
			isFirstLine = false;
	...
	private void _tokenizeString(Grammar grammar, /*OnigString*/ String lineText, boolean isFirstLine, int linePos, List<StackElement> stack, LineTokens lineTokens) {
