...
import fr.opensagres.language.textmate.core.css.CSSParser;
...
public class TMPresentationReconciler implements IPresentationReconciler {
	...
	class InternalListener
		...
		private IToken toToken(_editor.editors.tm.Token token) {
			...
			if (token.type != null) {
				if (parser != null) {
					// Use CSS Parser
					CSSStyle style = parser.getBestStyle(Arrays.asList(token.type.split("[.]")));
					if (style != null) {
						Token t = tokenMaps.get(style);
						if (t != null) {
							return t;
						}
					}
				...
				if (token.type.contains("meta") && token.type.contains("type") && token.type.contains("parameter")
						&& token.type.contains("variable")) {
	...
	private Map<CSSStyle, Token> tokenMaps;
	...
	public TMPresentationReconciler(ColorManager manager) {
		...
		metaTypeAnnotationToken = new Token(
				new TextAttribute(manager.getColor(IXMLColorConstants.META_TYPE_ANNOTATION)));
		metaParameterTypeVariableToken = new Token(
				new TextAttribute(manager.getColor(IXMLColorConstants.META_PARAMETER_TYPE_VARIABLE)));
		...
		//if (tokenMaps == null) {
			...
			try {
				parser = new CSSParser(TMPresentationReconciler.class.getResourceAsStream("style.css"));
				for (CSSStyle style : parser.getStyles()) {
					RGBColor color = style.getColor();
					if (color != null) {
						RGB rgb = toRGB(color);
						tokenMaps.put(style, new Token(new TextAttribute(manager.getColor(rgb))));
					}
				}
			} catch (Exception e) {
				// TODO Auto-generated catch block
	...
	public static RGB toRGB(RGBColor rgbColor) {
		if (rgbColor == null)
			return null;
